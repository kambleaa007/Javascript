...
public class DeleteEmployeeTest {
   ...
   @Test
    public void deleteEmployeeTest() throws Exception{
		...
		Employee employee = new Employee("Jack",1003, 190011.1,1002);
		when(empServiceIMPL.deleteEmployee(1003)).thenReturn(employee);
		...
		//Step6: Extract the actual response content and response status from MvcResult 
		//obtained above and compare with the expected response content and expected response status
		...
		verify(empServiceIMPL,times(1)).deleteEmployee(1003); 
    ...
    public void deleteInvalidEmployeeTest() throws Exception{
	  ...
	  //Step3: Use MockHttpServletRequestBuilder to create a request
  	  MockHttpServletRequestBuilder request= MockMvcRequestBuilders.delete(uri,1006)
  			  //ResponseContentType what test case/test client expects in return
  			  .accept(MediaType.APPLICATION_JSON) ;
  	  
  	  //Step4: Define the Mocking call for the mocked object created in Step1 and provided to controller 
	  when(empServiceIMPL.deleteEmployee(1006)).thenReturn(null);
	  ...
	  //Step7: Verify if the Controller is able to delegate the call to the mock
	  verify(empServiceIMPL,times(1)).deleteEmployee(1006); 
	  Assert.assertTrue(finalResult!=null);
	  Assert.assertTrue(finalResult.length()==0);
